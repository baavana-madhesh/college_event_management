...
export default function StaffDashboard() {
  ...
  const [regList, setRegList] = useState([]);  // for selected event in Registrations
  const [selectedEvent, setSelectedEvent] = useState(null);

  useEffect(()=>{
    api.staffStats(user.id).then(d=>{ if(d.success) setStats(d); });
    reloadMyEvents();
  },[]);
  ...
  const createEvent = async () => {
    ...
    if (res.success) { setForm({ title:"", description:"", date:"", capacity:50 }); reloadMyEvents(); setTab("overview"); }
  };
  ...
  const mark = async (student_id, status) => {
    await api.markAttendance({ event_id: selectedEvent, student_id, status, marked_by: user.id });
  ...
  return (
    <div className="container">
      ...
      {tab==="overview" && (
        <div style={{marginTop:16}} className="row cols-3">
          <div className="card">
            ...
            <div className="muted">{myEvents.filter(e=>e.status==='active').length} active</div>
          </div>
          <div className="card">
          ...
          <div className="card" style={{gridColumn:'1/-1'}}>
            ...
            {myEvents.slice(0,3).map(e=>(
              <div key={e.id} style={{display:'flex', justifyContent:'space-between', padding:'10px 0', borderTop:'1px solid #eee'}}>
                ...
                <div><span className="badge" style={{background:e.status==='active'?'#111827':'#9ca3af'}}>{e.status}</span></div>
              </div>
            ))}
          </div>
        </div>
          ...
          {myEvents.map(e=>(
            <div key={e.id} style={{display:'flex', justifyContent:'space-between', padding:'10px 0', borderTop:'1px solid #eee'}}>
              ...
              <div style={{display:'flex', gap:8}}>
                <button className="btn light" onClick={()=>{ setTab("registrations"); loadRegistrations(e.id); }}>Registrations</button>
      ...
      {tab==="registrations" && (
        <div className="card" style={{marginTop:16}}>
          <div className="h2">Registrations {selectedEvent ? `#${selectedEvent}`: ''}</div>
          {!selectedEvent && <div className="muted">Open “My Events” and click “Registrations”.</div>}
          {selectedEvent && regList.map(s=>(
            <div key={s.student_id} style={{display:'flex', justifyContent:'space-between', padding:'10px 0', borderTop:'1px solid #eee'}}>
              ...
              <div style={{display:'flex', gap:8}}>
                <button className="btn light" onClick={()=>mark(s.student_id,'present')}>Mark Present</button>
                <button className="btn light" onClick={()=>mark(s.student_id,'absent')}>Mark Absent</button>
              </div>
            </div>
          ))}
          {selectedEvent && regList.length===0 && <div className="muted">No students yet.</div>}
        </div>
      )}
      ...
      {tab==="create" && (
        <div className="card" style={{marginTop:16}}>
          ...
          <input className="input" type="number" value={form.capacity} onChange={e=>setForm({...form,capacity:parseInt(e.target.value||50)})}/>
          <div style={{height:16}}/>
